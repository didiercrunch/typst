...
struct AssetMirror<'a> {
    root: &'a Path,
}

impl AssetMirror<'_> {
    fn new(path: &Path) -> AssetMirror {
...
mod tests_asset_mirror {
    ...
    fn path_for_basic() {
        ...
        let asset_mirror = AssetMirror::new(Path::new("/tmp/typst"));
...
pub struct HTTPRemoteAssetFetcher<'a> {
    _agent: ureq::Agent,
    mirror: AssetMirror<'a>,
}

impl HTTPRemoteAssetFetcher<'_> {
    pub fn new(root: &Path) -> HTTPRemoteAssetFetcher {
...
mod tests_http_remote_asset_fetcher {
    ...
    fn create_http_remote_asset_fetcher() {
        HTTPRemoteAssetFetcher::new(Path::new("/tmp/toto"));
    }
